  ...
  L(_L), r(_r), N(_N), orientation(_orientation), position(_pos), dt(dt)
{
  ...
  float dTheta = 2.0 * M_PI / res;

  // Vectors in homogeneous coordinates
  cv::Point3f start(0, 0, 0);
  cv::Point3f end(0, r, 0);
  ...
  cv::Mat offset_mat = (cv::Mat_<float>(4, 1) << offset, 0, 0, 0);
  ...
  cv::Mat coil_transform(cv::Size(4, 4), CV_32FC1, cv::Scalar(0));
    ...
    coil_transform.at<float>(1, 1) = cos(dTheta);
    coil_transform.at<float>(2, 1) = sin(dTheta);
    coil_transform.at<float>(1, 2) = -sin(dTheta);
    coil_transform.at<float>(2, 2) = cos(dTheta);
    // Scale
  ...
  for(int i = 0; i < vec_num; i++){
    ...
    cv::Mat end_mat_rotated = orientation_transform * (end_mat + offset_mat);
    cv::Mat start_mat_rotated = orientation_transform * (start_mat + offset_mat);
...
cv::Vec2d Coil::getFieldVectorAtPos(cv::Vec2d pos){
